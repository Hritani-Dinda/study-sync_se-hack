...
export default function DashboardClient({ firstName, courses }: DashboardClientProps) {
  ...
  const getGoogleCalendarLink = (title: string, date: string, description = ""): string => {
    ...
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${startFormatted}/${endFormatted}&details=${encodeURIComponent(description)}`
  }
                ...
                <div className="space-y-2">
                  ...
                  <div className="border p-3 rounded-md">
                    <div className="flex justify-between items-center">
                      ...
                      <a 
                        href={getGoogleCalendarLink("Database Assignment", "Tomorrow, 11:59 PM", "CS301 - Complete your assignment")}
                        target="_blank"
            ...
            <Card className="col-span-3">
              ...
              <CardContent>
                <div className="space-y-4">
                  {[{ title: "Database Assignment", date: "Tomorrow, 11:59 PM", urgent: true, courseId: "cs301" }].map((item, i) => (
                    <div key={i} className="flex items-center">
                      ...
                      <div className="ml-auto flex items-center space-x-2">
                        {item.urgent && <Badge variant="destructive" className="mr-2">Urgent</Badge>}
                        <Button asChild size="sm" variant="outline">
                          <Link href={`/courses/${item.courseId}`}>View</Link>
                        </Button>
                        <Button asChild size="sm" variant="ghost">
                          <a 
                            href={getGoogleCalendarLink(item.title, item.date, `Work on ${item.title}`)}
                            target="_blank"
                            rel="noopener noreferrer"
