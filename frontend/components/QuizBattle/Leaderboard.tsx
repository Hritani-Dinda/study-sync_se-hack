...
export default function Leaderboard() {
  ...
  return (
    <div className="p-6 max-w-md mx-auto bg-white rounded-xl shadow-md">
      <h2 className="text-xl font-bold mb-4">{quizComplete ? "Final Results" : "Live Leaderboard"}</h2>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {leaderboard.map((player, index) => (
                <tr key={player.userId || index} className={index === 0 ? "bg-yellow-50" : ""}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold">{player.score}</td>
                </tr>
              ))}
            </tbody>
          </table>
